...
class RiskSpace(Measure):
    ...
    def measure_one(self, woman):
        ...
        if "prospect" in player_type:
            LOG.debug(
                "Measuring valuespace for player %d, type %d, rule %s" % (hash(woman), woman.player_type, str(woman)))
            res = map(lambda signal: woman.cpt_value(woman.collect_prospects(signal)), woman.signals)
        elif "lexic" in player_type:
            LOG.debug(
                "Measuring signalspace for player %d, type %d, rule %s" % (hash(woman), woman.player_type, str(woman)))
            res = woman.signal_search()
        else:
            LOG.debug(
                "Measuring riskspace for player %d, type %d, rule %s" % (hash(woman), woman.player_type, str(woman)))
